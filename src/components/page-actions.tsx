...
// Localized replacement for Fumadocs's `MarkdownCopyButton` / `ViewOptionsPopover`
// (fumadocs-ui/layouts/notebook/page). Those bake their labels in as English
// literals with no prop or i18n hook, so the only way to translate "Copy
// Markdown", "Open", "View as Markdown", "Open in ChatGPT", etc. is to
// re-implement them. Behaviour, styling, and icons mirror the originals; only
// the strings are read from the active locale (falling back to English). Keep
// this in sync if Fumadocs changes its page-actions markup on upgrade.
...
> = {
  en: {
    ...
    askPrompt: (url) => `Read ${url}, I want to ask questions about it.`,
  },
  cs: {
    ...
    openInScira: 'Otevřít ve Scira AI',
    openInChatGPT: 'Otevřít v ChatGPT',
    openInClaude: 'Otevřít v Claude',
    openInCursor: 'Otevřít v Cursoru',
    askPrompt: (url) => `Přečti si ${url}, chci se na něj ptát.`,
  },
};
...
export function MarkdownCopyButton({
  ...
  return (
    ...
    >
      {checked ? <Check /> : <Copy />}
      {props.children ?? labels.copyMarkdown}
    </button>
  );
}
...
export function ViewOptionsPopover({
  ...
  const items = useMemo(() => {
    ...
    return [
      ...
      markdownUrl && {
        title: labels.viewAsMarkdown,
        href: markdownUrl,
        icon: <TextIcon />,
      },
      {
        title: labels.openInScira,
  ...
  }, [githubUrl, markdownUrl, pathname, labels]);
